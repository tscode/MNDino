...
function Makie.process_interaction(seg::SegmentInteraction, event::ScrollEvent, ax)
  mask = seg.mask[]
  if event.y < 0
    ImageMorphology.erode!(mask, r=3)
  else
    ImageMorphology.dilate!(mask, r=3)
  end
  notify(seg.mask)
  return
end
...
function _keyboard_control!(wctx)
  for index in 1:wctx[:nchannels]
    ...
    register_interaction!(ax, :keyboard) do _::KeysEvent, ax
      ...
      elseif event.key in [Keyboard.left_alt, Keyboard.right_alt]
        if event.action == Keyboard.press
          wctx[:mask_on][] = false
        elseif event.action == Keyboard.release
          wctx[:mask_on][] = true
...
function plotwidget(widget::ChannelViewMaskWidget, layout, wctx, theme)
  rowgap!(layout, 2, 20)
  ...
  on(pointer_button.clicks) do _
    wctx[:pointer_on][] = !wctx[:pointer_on][]
  end

  on(mask_button.clicks) do _
    wctx[:mask_on][] = !wctx[:mask_on][]
  end
